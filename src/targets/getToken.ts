...
async function getWithClientCredentials(
  ...
  const incomingScopes =
    typeof params.scope === "string" ? params.scope.split(" ") : [];
  if (!userPoolClient) {
    throw new NotAuthorizedError();
  }
  if (
  ...
  const scopes: string[] = userPoolClient.AllowedOAuthScopes
    ? userPoolClient.AllowedOAuthScopes
    : [];
  const requestedScopes: string[] = incomingScopes
    .filter((k) => typeof k === "string")
    .map((k) => k.toLowerCase().trim());
  const unmatchedScopes: string[] = requestedScopes.filter(
    (requestedScope) => !scopes.includes(requestedScope)
  );

  if (unmatchedScopes.length) {
    throw new NotAuthorizedError(
      `Invalid scopes: ${unmatchedScopes.join(" ")}`
    );
  }
  ...
  const tokens = await services.tokenGenerator.generateWithClientCreds(
    ctx,
    userPoolClient,
    requestedScopes.length ? requestedScopes : ["aws.cognito.signin.user.admin"]
  );
  if (!tokens) {
    throw new NotAuthorizedError();
  }
